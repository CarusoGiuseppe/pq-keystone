...
int mbedtls_x509write_crt_der_tmp(mbedtls_x509write_cert *ctx, unsigned char *buf, size_t size,
    ...
    falcon_sign_dyn(rng, sig, &sig_len1, FALCON_SIG_CT, ctx->issuer_key->pk_ctx.priv_key, FALCON_PRIVKEY_SIZE(LOGN_TEST), hash, 64, tmp, FALCON_TMPSIZE_SIGNDYN(LOGN_TEST));
    sig_len = sig_len1;
