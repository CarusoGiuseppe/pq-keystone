...
int pk_set_falconpubkey(unsigned char **p, mbedtls_falcon_context *falcon){
    ...
    */
    my_memcpy(falcon->pub_key, *p, len);
    falcon->len = len;
    return 0;
...
int pk_set_falconprivkey(unsigned char **p, mbedtls_falcon_context *falcon){
    ...
    */
    my_memcpy(falcon->priv_key, *p, len);
    falcon->len = len;
    return 0;
...
int mbedtls_x509write_crt_der_tmp(mbedtls_x509write_cert *ctx, unsigned char *buf, size_t size,
    ...
    falcon_sign_dyn(rng, sig, &sig_len1, FALCON_SIG_CT, ctx->issuer_key->pk_ctx.priv_key, FALCON_PRIVKEY_SIZE(LOGN_PARAM), hash, 64, tmp, FALCON_TMPSIZE_SIGNDYN(LOGN_PARAM));
    sig_len = sig_len1;
    
    /* Move CRT to the front of the buffer to have space
     * for the signature. */
    my_memmove(buf, c, len);//in buf there is the pointer to the beginning of the cert
    c = buf + len;
    ...
    c = c2 - len;
    my_memmove(c, buf, len); //c points to the beginning of the signature
    len += sig_and_oid_len;
    MBEDTLS_ASN1_CHK_ADD(len, mbedtls_asn1_write_len(&c, buf, len));
    MBEDTLS_ASN1_CHK_ADD(len, mbedtls_asn1_write_tag(&c, buf,
                                                     MBEDTLS_ASN1_CONSTRUCTED |
