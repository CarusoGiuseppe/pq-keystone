...
typedef enum {
    ...
    KEYSTONE_SHA3,
    //FALCON
    //FALCON512_SHAKE256, //algorithm used internally into Falcon
    //FALCON1024_SHAKE256
...
*/
typedef struct mbedtls_falcon_context {
...
typedef struct mbedtls_pk_context {
    ...
    /*void **/mbedtls_falcon_context pk_ctx;                       /**< Underlying public key context  */
} mbedtls_pk_context;
...
//FALCON
typedef void mbedtls_falcon_restart_ctx;
...
//FALCON
void/* mbedtls_falcon_context*/ falcon_alloc_wrap(void);
//FALCON
void falcon_free_wrap(void *ctx);
//FALCON
int mbedtls_falcon_check_pub_priv(unsigned char* priv, unsigned char* pub, unsigned char* seed);
//FALCON
int falcon_check_pair_wrap(const void *pub, const void *prv, int (*f_rng)(void *, unsigned char *, size_t), void *p_rng);
//FALCON
int falcon_encrypt_wrap(void *ctx, const unsigned char *input, size_t ilen, unsigned char *output, size_t *olen, size_t osize, int (*f_rng)(void *, unsigned char *, size_t), void *p_rng);
//FALCON
int falcon_decrypt_wrap(void *ctx, const unsigned char *input, size_t ilen,unsigned char *output, size_t *olen, size_t osize, int (*f_rng)(void *, unsigned char *, size_t), void *p_rng);
...
//FALCON
int falcon_wrap(void *ctx,const unsigned char *hash, size_t hash_len,unsigned char *sig, size_t sig_size, size_t *sig_len);
//FALCON
...
int mbedtls_x509write_crt_der_tmp(mbedtls_x509write_cert *ctx, unsigned char *buf, size_t size,
                                int (*f_rng)(void *, unsigned char *, size_t), unsigned char *tmp, shake256_context *rng, unsigned char* sig);
int mbedtls_x509write_crt_der(mbedtls_x509write_cert *ctx, unsigned char *buf, size_t size, int (*f_rng)(void *, unsigned char *, size_t), void *p_rng);//, unsigned char* test, int *l_topass);
...
int x509_write_time(unsigned char **p, unsigned char *start, const char *t, size_t size);
...
int mbedtls_pk_sign_restartable(mbedtls_pk_context *ctx, mbedtls_md_type_t md_alg, const unsigned char *hash, size_t hash_len, unsigned char *sig, size_t sig_size, size_t *sig_len, int (*f_rng)(void *, unsigned char *, size_t), void *p_rng);
...
int mbedtls_x509_crt_parse_der_internal(mbedtls_x509_crt *chain,  unsigned char *buf, size_t buflen, int make_copy, mbedtls_x509_crt_ext_cb_t cb, void *p_ctx);
...
int mbedtls_x509_set_extension(mbedtls_asn1_named_data *head, const char *oid, size_t oid_len, int critical, /*const*/ unsigned char *val, size_t val_len, int *ne);
...
int mbedtls_asn1_get_alg_mod(unsigned char **p, const unsigned char *end, mbedtls_asn1_buf_no_arr *alg, mbedtls_asn1_buf *params);  
int mbedtls_x509_get_alg_mod(unsigned char **p, const unsigned char *end, mbedtls_x509_buf *alg, mbedtls_x509_buf *params);
int mbedtls_x509_get_sig_alg_mod(const mbedtls_x509_buf_crt *sig_oid, const mbedtls_x509_buf *sig_params, mbedtls_md_type_t *md_alg, mbedtls_pk_type_t *pk_alg, void **sig_opts);
int mbedtls_asn1_get_bool(unsigned char **p, const unsigned char *end, int *val);
int x509_write_extension_mod(unsigned char **p, unsigned char *start, mbedtls_asn1_named_data ext);
int mbedtls_x509_write_extensions_mod(unsigned char **p, unsigned char *start, mbedtls_asn1_named_data *arr_exte, int ne);
int x509_get_crt_ext(unsigned char **p, const unsigned char *end, mbedtls_x509_crt *crt, mbedtls_x509_crt_ext_cb_t cb, void *p_ctx);
...
int mbedtls_x509write_crt_set_dice_tcbInfo(mbedtls_x509write_cert *ctx, dice_tcbInfo info_struct, int dim, unsigned char buf[], size_t buf_size);
int x509_get_dice_tcbInfo(unsigned char **p, const unsigned char *end, dice_tcbInfo* info_struct);  
int setting_tcbInfo(dice_tcbInfo* dice_tcbInfo, unsigned char vendor[], int l_ven, unsigned char model[], int l_m, unsigned char version[], int l_ver, int svn, int layer, int index, unsigned char flags[], int l_f, unsigned char vendor_info[], int l_vf, unsigned char type[], int l_t, measure measures[], int l_mea);   
...
int mbedtls_x509_get_ext(unsigned char **p, const unsigned char *end,mbedtls_x509_buf_crt *ext, int tag);
int x509_get_uid(unsigned char **p,const unsigned char *end, mbedtls_x509_buf_crt *uid, int n);
...
//FALCON
int mbedtls_asn1_write_falcon_identifier(unsigned char **p, const unsigned char *start, const char *oid, size_t oid_len);
//FALCON
int mbedtls_asn1_get_alg_falcon(unsigned char **p, const unsigned char *end, mbedtls_asn1_buf *alg);
